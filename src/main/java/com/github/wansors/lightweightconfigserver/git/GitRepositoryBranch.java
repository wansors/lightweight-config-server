...
public class GitRepositoryBranch {
	...
	public void init() throws IOException, InvalidRemoteException, TransportException, GitAPIException {
		if (branchFolder == null) {
			branchFolder = Files.createTempDirectory("tmpgit").toFile();
			CloneCommand cloneCommand = Git.cloneRepository();
			cloneCommand.setDirectory(branchFolder).setCloneAllBranches(true).setURI(gitConf.uri());
			if (gitConf.isAuthenticationEnabled()) {
				cloneCommand.setCredentialsProvider(new UsernamePasswordCredentialsProvider(
						gitConf.username().orElse(null), gitConf.password().orElse(null)));
			}
			cloneCommand.call();
	...
	protected boolean shouldPull() {
		if (gitConf.refreshRate() > 0 && System.currentTimeMillis() - lastRefresh < (gitConf.refreshRate() * 1000)) {
			return false;
		}
